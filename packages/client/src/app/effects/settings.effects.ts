...
export class SettingsEffects {
  ...
  saveTranscriptionSettings$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(({ transcription }) => {
        // Write the form's value directly, matching every other setting effect.
        // The previous implementation merged against the store's transcription
        // slice, which could persist a stale enabled:false when the store hadn't
        // caught up with the toggle — the setting looked saved in-session but
        // reverted on reload.
